...
/// </summary>
public class TrackManager : MonoBehaviourPun, IPunObservable
{
    [SerializeField] private Transform _container;
    [SerializeField] private BarrierSlowdown _barrierSlowdown;
    [SerializeField] private BarrierWater _barrierWater;
    [SerializeField] private BarrierSphere _barrierSphere;
    [SerializeField] private FireRoad _fireRoad;
    ...
    public float maxSpeedMultiplayer = 10.0f;
    ...
    public bool m_Rerun;     // This lets us know if we are entering a game over (ads) state or starting a new game (see GameState)

    public bool m_IsTutorial; //Tutorial is a special run that don't chance section until the tutorial step is "validated" by the TutorialState.
    ...
    [PunRPC]
    public void SetCharacterParent(int viewId)
    {
        ...
        if (view != null)
        {
            ...
            view.GetComponent<Character>().StartPlayAnimation();
        }
    }

    [PunRPC]
    public void StartRunning(int viewId)
    {
        PhotonView view = PhotonNetwork.GetPhotonView(viewId);

        if (view != null)
            view.GetComponent<Character>().StartRun();
    }
    ...
    IEnumerator WaitToStart()
    {
        if (PhotonNetwork.IsConnected)
        {
            ...
            photonView.RPC(nameof(SetCharacterParent), RpcTarget.OthersBuffered, characterController.character.photonView.ViewID);
        }
        ...
        if (PhotonNetwork.IsConnected)
            photonView.RPC(nameof(StartRunning), RpcTarget.OthersBuffered, characterController.character.photonView.ViewID);
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            //{
            //    if (m_TrackSeed != -1)
            //        _random = new Random(m_TrackSeed);
            //    else
            ...
            if (PhotonNetwork.IsConnected)
            {
                player = PhotonNetwork.Instantiate("Player", Vector3.zero, Quaternion.identity).GetComponent<Character>();
            }
            else
            {
                ...
                player = op.Result.GetComponent<Character>();
            }
            ...
            else
            {
                if (PhotonNetwork.IsConnected)
                {
                    m_CurrentThemeData = ThemeDatabase.GetThemeData("Multiplayer");
                }
                else
                {
    ...
    void Update()
    {
        if (PhotonNetwork.IsConnected)
        {
            if (PhotonNetwork.IsMasterClient)
            {
                _positionZ = characterController.transform.position.z;
            }
        }
                ...
                if (cloud != null)
                {
                    ...
                    obj.transform.localPosition =
                        Vector3.up * (currentTheme.cloudMinimumDistance.y +
                                      (value - 0.5f) * currentTheme.cloudSpread.y)
                        + Vector3.forward * (lastZ + (value - 0.5f) * currentTheme.cloudSpread.z)
                        + Vector3.right * (currentTheme.cloudMinimumDistance.x +
                                           (value - 0.5f) * currentTheme.cloudSpread.x);

                    obj.transform.localScale = obj.transform.localScale * (1.0f + (value - 0.5f) * 0.5f);
                    obj.transform.localRotation = Quaternion.AngleAxis(value * 360.0f, Vector3.up);
                    _parallaxRootChildren++;
                }
            }
        }
        ...
        if (!m_IsTutorial)
        {
            float maxSpeed = PhotonNetwork.IsConnected ? maxSpeedMultiplayer : this.maxSpeed;
...
//"level" in our game are milestone the player have to reach : one every 300m
    ...
    public void CreateBarrierSlowdown()
    {
        Vector3 position = new Vector3(characterController.character.transform.position.x, 0.5f, _positionZ + 75);

        photonView.RPC(nameof(CreateNetworkBarrierSlowdonw), RpcTarget.AllBuffered, position, PhotonNetwork.LocalPlayer.ActorNumber);
    }

    public void CreateFireRoad()
    {
        Vector3 position = new Vector3(characterController.character.transform.position.x, 0.5f, _positionZ + 75);

        photonView.RPC(nameof(CreateNetworkFireRoad), RpcTarget.AllBuffered, position, PhotonNetwork.LocalPlayer.ActorNumber);
    }

    public void CreateBarrierWater()
    {
        Vector3 position = new Vector3(characterController.character.transform.position.x, 0.5f, _positionZ + 75);

        photonView.RPC(nameof(CreateNetworkBarrierWater), RpcTarget.AllBuffered, position, PhotonNetwork.LocalPlayer.ActorNumber);
    }
    ...
    public void CreateSphere()
    {
        ...
        photonView.RPC(nameof(CreateNetworkSphereWater), RpcTarget.AllBuffered, position, PhotonNetwork.LocalPlayer.ActorNumber);
    }

    [PunRPC]
    public void CreateNetworkBarrierSlowdonw(Vector3 position, int actorNumber)
    {
        var createdBarrier = Instantiate(_barrierSlowdown, position, Quaternion.identity, characterController.transform);
        createdBarrier.Init(actorNumber);
    }

    [PunRPC]
    public void CreateNetworkFireRoad(Vector3 position, int actorNumber)
    {
        var createdBarrier = Instantiate(_fireRoad, position, Quaternion.identity, characterController.transform);
        createdBarrier.Init(actorNumber);
    }

    [PunRPC]
    public void CreateNetworkBarrierWater(Vector3 position, int actorNumber)
    {
        var createdBarrier = Instantiate(_barrierWater, position, Quaternion.identity, characterController.transform);
        createdBarrier.Init(actorNumber);
    }

    [PunRPC]
    public void CreateNetworkSphereWater(Vector3 position, int actorNumber)
    {
        var createdBarrier = Instantiate(_barrierSphere, position, Quaternion.identity, characterController.transform);
        createdBarrier.Init(actorNumber);
    }
    ...
    private IEnumerator SpawnFromAssetReference(AssetReference reference, TrackSegment segment, int posIndex)
    {
        ...
        if (obj != null)
        {
            ...
            if (obstacle != null)
                yield return obstacle.Spawn(segment, segment.obstaclePositions[posIndex], Random.seed);
        }
    }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            stream.SendNext(m_TimeToStart);
            stream.SendNext(m_Speed);
            stream.SendNext(_positionZ);
        }
        else
        {
            m_TimeToStart = (float)stream.ReceiveNext();
            m_Speed = (float)stream.ReceiveNext();
            _positionZ = (float)stream.ReceiveNext();
        }
    }
}